...
const authController = {
    ...
    registerUser: async (req, res) => {
        try {
            ...
            req.getConnection((err, conn) => {
              const kq =  conn.query(Queryuser.registerUser, {
    ...
    generateAccessToken: (user) => {
        return jwt.sign({
            id: user.id,
            admin: user.admin,
        }, process.env.JWT_ACCESS_KEY, { expiresIn: '200s' });
    },
    ...
    loginUser: async (req, res) => {
        try {
            ...
            req.getConnection((err, conn) => {
                ...
                conn.query(Queryuser.loginUser, req.body.username, async (err, result) => {
                    if (err) {
                    ...
                    // Lưu RefreshToken vào mảng (nếu cần)
                    refreshTokens.push(refreshtoken);
    
                    // Gửi AccessToken và thông tin người dùng (loại bỏ mật khẩu) cho client
